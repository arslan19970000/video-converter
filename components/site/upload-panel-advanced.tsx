...
import { Upload, Download, FileVideo, RefreshCcw, Loader2, Wand2, Settings, Film } from "lucide-react"
...
export function UploadPanelAdvanced() {
  ...
  const handleOptionsChange = (newOptions: Partial<ConversionOptions>) => {
    ...
    setSelectedPreset(undefined) // Clear preset when manual changes
  }
  ...
  const startConversion = useCallback(async () => {
    ...
    try {
      ...
      if (mode === "merge") {
        ...
        // Single file mode with all options
        const conversionOptions: ConversionOptions = {
          ...options,
          trim: trim || undefined,
        }

        outputBlob = await convertVideo(file!, conversionOptions, (progressValue) => {
          setProgress(progressValue)
        })
      }
                ...
                <div className="space-y-4">
                  ...
                  <Tabs defaultValue="presets" className="w-full">
                    ...
                    <TabsContent value="presets" className="mt-4">
                      <FormatPresets selectedPreset={selectedPreset} onSelect={handlePresetSelect} />
                    </TabsContent>
                    ...
                    <TabsContent value="trim" className="mt-4">
                      {trim && (
                        <TrimTimeline
                          ...
                          trim={trim}
                          onChange={setTrim}
                          onReset={() => setTrim({ start: 0, end: duration })}
                        />
                      )}
                    </TabsContent>

                    <TabsContent value="edit" className="mt-4">
                      <EditingTools options={options} onChange={handleOptionsChange} />
                    </TabsContent>

                    <TabsContent value="advanced" className="mt-4">
                      <AdvancedOptions options={options} onChange={handleOptionsChange} />
                    </TabsContent>
                  </Tabs>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {downloadUrl && !converting && (
                        <Button
                          ...
                          variant="outline"
                          size="lg"
                          className="flex-1 sm:flex-none border-2 hover:border-green-500 hover:bg-green-500/10"
                        >
                          <a
                            download={`${file.name.split(".")[0]}.${options.outputFormat}`}
                            href={downloadUrl}
            ...
            <div className="space-y-4">
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="grid gap-2 sm:col-span-3">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleOptionsChange({ outputFormat: value })}
                  >
                    <SelectTrigger className="h-auto py-3 px-4 rounded-xl border-2 bg-background/50 backdrop-blur-sm font-medium text-base shadow-sm">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {downloadUrl && !converting && (
                  ...
                  >
                    <a
                      download={`merged.${options.outputFormat}`}
                      href={downloadUrl}
                      className="inline-flex items-center gap-2"
                    >
